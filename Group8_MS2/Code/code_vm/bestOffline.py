...
def fetchData(sensor_id):
  ...
  while len(response_parsed['body']['hits']['hits'])>0:
    ...
    if (data.index.min()<(datetime(today.year, today.month, today.day-1))):
        break
    i+=1
...
def main(params):
    ...
    combined = combined[yesterday.strftime("%Y-%m-%d")]

    combined["pred_arima"] = (combined["pred_arima"]).astype(int)
    combined["pred_linear"] = (combined["pred_linear"]).astype(int)

    score_arima = accuracy_score(combined["value"],combined["pred_arima"])
    score_linear = accuracy_score(combined["value"],combined["pred_linear"])

    if (score_arima>score_linear):
        pushMQTT("bestOffline",combined, 1)
    else:
        pushMQTT("bestOffline", combined, 2)
    df = pd.DataFrame(index=[pd.Timestamp(yesterday)], data=[score_arima])
    pushMQTT("accuracyARIMA",df,0)
    df = pd.DataFrame(index=[pd.Timestamp(yesterday)], data=[score_linear])
    pushMQTT("accuracyLINEAR",df,0)
